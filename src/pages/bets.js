...
const Bets = () => {
  ...
  useEffect(() => {
    ...
    if (lastChosenUser) {
      ...
      setTempUser(lastChosenUser);
    }

    const unsubscribeAuth = auth.onAuthStateChanged((user) => {
      if (user?.displayName) {
        setSelectedUser(user.displayName);
        setTempUser(user.displayName);
      }
    });
  ...
  const modalButtonStyle = {
    backgroundColor: "#DC3545", color: "white", border: "none", padding: "10px 20px", borderRadius: "15px", fontWeight: "bold", marginTop: "15px", cursor: "pointer", margin: "5px"
  };
  ...
  return (
    <div className="fade-in" style={{ textAlign: 'center', color: 'yellow' }}>
      ...
      <select
        style={{ margin: '1px', backgroundColor: 'pink', fontWeight: 'bold', fontFamily: 'Rubik', pointerEvents: 'none' }}
        value={selectedUser}
        disabled={true}
      >
        {Object.keys(usersData).map((user) => (
      ...
      >
        <FontAwesomeIcon icon={faEdit} style={{ marginRight: '3px' }} /> Zmień
      </button>
